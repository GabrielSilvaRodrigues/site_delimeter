import React from 'react';
import './Sobre.css';

const Sobre = () => {
  return (
    <main className="sobre-main">
      <section className="container-main">
        <div className="container-main-image">
          <img src="/assets/images/almoço.jpg" alt="Pessoas em uma mesa distribuindo refeições" />
        </div>
      </section>
      
      <section className="about">
        <h1>Sobre</h1>
        <div className="caixas">
          <div className="caixa">
            <p className="about-paragrafo">
              O <span>Deliméter</span> é um projeto voltado à nutrição de pessoas carentes, 
              unindo tecnologia e alimentação saudável para combater a insegurança alimentar. 
              Ele visa oferecer soluções inovadoras para facilitar o acesso a alimentos nutritivos, 
              promover educação alimentar e otimizar a distribuição de recursos para comunidades 
              em situação de vulnerabilidade.
            </p>
            
            <p className="about-paragrafo">
              O nome <span>Deliméter</span> é inspirado na deusa <span>Deméter</span>, 
              conhecida como a deusa da agricultura na mitologia grega. A parte "Deli" do nome 
              faz referência a <span>delimitação</span>, que é o principal objetivo deste site: 
              delimitar e acompanhar a sua alimentação.
            </p>
            
            <p>Alguns termos que você pode encontrar pelo site:</p>
            <ul className="about-lista">
              <li>
                <strong>GEB:</strong> Gasto Energético Basal (Quantidade de calorias gastas para te manter vivo);
              </li>
              <li>
                <strong>GET:</strong> Gasto Energético Total (Quantidade de calorias gastas somadas ao GEB e suas atividades físicas);
              </li>
              <li>
                <strong>Macronutrientes:</strong> Nutrientes utilizados em grandes quantidades pelo nosso organismo (CHO, PTN, LIP);
              </li>
              <li>
                <strong>CHO:</strong> Carboidrato, conhecido como açúcar, pode ser composto por: glicose, frutose e lactose;
              </li>
              <li>
                <strong>PTN:</strong> Proteína, conhecida por ser construtora, com maior disponibilidade vindo de produtos de origem animal como: carnes, leite e ovos;
              </li>
              <li>
                <strong>LIP:</strong> Lípidios, conhecidos como gordura, tem uma grande variedade de alimentos que são ricos em gorduras como: óleos vegetais, peixes e castanhas;
              </li>
            </ul>
          </div>
          
          <div className="delimeter-image">
            <img src="/assets/images/delimeter.png" alt="Deusa delimeter" />
          </div>
        </div>
      </section>
    </main>
  );
};

export default Sobre;